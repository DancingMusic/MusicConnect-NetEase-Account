...
当 `login({ intent: "status" })` 已认证时，连接器 MUST 通过同一受限账号资料操作返回
非敏感的 `user.id`、`user.name`、`user.avatarUrl` 以及标准 `membership` 摘要。昵称、头像
URL 和官方 `vipType` 仅用于宿主展示；Cookie、Token、到期凭据和原始账号响应不得返回。
资料接口暂时失败时，状态仍可保持已认证，但不得伪造头像、昵称或会员等级。
...
- 修复账号曲库、封面、“我的歌单”与账号展示摘要的版本为 `0.3.1`。
